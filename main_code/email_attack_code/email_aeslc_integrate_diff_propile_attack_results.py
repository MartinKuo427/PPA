...
class Client:
    def __init__(self):
        ...
        self.EXPOSE_EMAIL_ITEM = []

        # self.twodim_dict = {{}}
        self.database_dict  = {
            'expose_item': {
                'EMAIL': self.EXPOSE_EMAIL_ITEM
            }
        }        
...
def save_record_to_excel(output_filepath, person_pii_dict, privacy_ground_truth_table):
    ...
    for person_index in range(len(privacy_ground_truth_table)):
        ...
        item = person_pii.database_dict["expose_item"]["EMAIL"] if len(person_pii.database_dict["expose_item"]["EMAIL"]) > 0 else "-"
        EXPOSE_EMAIL_ITEM.append(item)
    ...
    df = pd.DataFrame({
        ...
        'EXPOSE_EMAIL_ITEM': EXPOSE_EMAIL_ITEM,
    }, columns=['NAME', 'CATEGORY', 'TEMPLATE_TYPE', 'TEMPLATE', 'TEMPLATE_OUTPUT', 'AWS_COMPREHEND_OUTPUT', 'EXPOSE_EMAIL_ITEM'])
